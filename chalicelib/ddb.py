...
class DdbChat():
    ...
    def putComment(self, table, name, comment, chat_room):
        logging.info('PutComments params : %s %s %s %s',
                     table, name, comment, chat_room)
    ...
    def getRangeComments(self, table, chat_room, position):
        logging.info('getRabgeComments params : %s %s %s',
                     table, chat_room, str(position))
        ...
        response = table.query(
            IndexName='chat_room_time_idx',
            Select='ALL_ATTRIBUTES',
            KeyConditionExpression=Key('chat_room').eq(
                chat_room) & Key('time').gt(position),
            ScanIndexForward=False
        )
...
if __name__ == "__main__":
    ...
    for index, item in enumerate(list):
        logging.info('id: ' + str(index) + ' name:' +
                     item['name'] + ' time:' + str(item['time']) + ' comment:' + item['comment'])

    result = ddb.getAllComments(table, chat_room)
    for index, item in enumerate(result):
        logging.info('ALL Result ' + 'id: ' + str(index) + ' name:' +
                     item['name'] + ' time:' + str(item['time']) + ' comment:' + item['comment'])
    ...
    for index, item in enumerate(result):
        logging.info('RANGE Result ' + 'id: ' + str(index) + ' name:' +
                     item['name'] + ' time:' + str(item['time']) + ' comment:' + item['comment'])
